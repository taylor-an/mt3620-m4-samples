...
static void SELECT(void)
{
  ...
  status = SPIMaster_SelectEnable(card->interface, true);
...
static void DESELECT(void)
{
  ...
  status = SPIMaster_SelectEnable(card->interface, false);
  if (status != ERROR_NONE) {
...
static void SPI_TxByte(uint8_t data)
{
  ...
  status = SPIMaster_TransferSequentialAsync(card->interface, &transfer, 1, 0);
  if (status != ERROR_NONE) {
